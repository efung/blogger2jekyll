    ...
    function templatePosts() {
      postsArr.forEach(function (post) {
        ...
        if (!opts.skipComments) {
          post.comments.forEach(function (comment) {
            subcontents.push(
              ...
              , '    ' + comment.published.toDateString()
              , '  </div>'
              , '  </div>'
            );
        ...
        if ( post.thumbnail )
          ...
          var imgSrcMatch = post.content.match(/<img[^>]+src="([^"\s]+)"/);
          if (imgSrcMatch && imgSrcMatch[1]) {
            var thumbnail;
            if ( imgSrcMatch[1].match(/_[a-z]\./) )
              thumbnail = imgSrcMatch[1].replace(/_[a-z]\./, "_t.")
            else
              thumbnail = imgSrcMatch[1].replace(/\.(jpg|png)/, "_t.$1")

            yaml.push( 'thumbnail: "' + thumbnail + '"' );
          }
        }
        ...
        if ( typeof post.category != 'undefined' ) {
          ...
          yaml.push( 'tags: [' + post.category.join(', ') + ']' );
        }
        else
        ...
        var plainContent = post.content.replace(/<\/?[^>]+(>|$)/g, "")
                                       .replace(/.*Notes: /, "")
                                       .replace(/'/g, "\\'")
                                       .replace(/"/g,"\\\"")
                                       .replace(/&nbsp;/," ");
        var EXCERPT_LENGTH = 300;
        yaml.push( 'excerpt: "' + plainContent.substring(0, plainContent.indexOf(" ", EXCERPT_LENGTH)) + '"' )

        yaml.push( 'comments: true' );

        yaml.push( 'redirect_from: /blogspot/' + post.uuid + '.html');
